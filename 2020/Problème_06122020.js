...
const answerList = input.split('\n')

const answerByGroupList = []

let currString = ''

for (const answer of answerList) {
    if (answer === '') {
        answerByGroupList.push(currString)
        currString = '' 
    } else {
        currString = `${currString}${answer}`
    }
}
...
const uniqAnswerByGroupList = []

for (const answer of answerByGroupList) {
    const currAnswer = []
    for (let i=0; i < answer.length; ++i) {
        if (currAnswer.findIndex(x => x === answer.charAt(i)) === -1) {
            currAnswer.push(answer.charAt(i))
        }
    }
    uniqAnswerByGroupList.push(currAnswer.join(''))
}

console.log(uniqAnswerByGroupList.join('\n'))
console.log(`Total de réponse : ${uniqAnswerByGroupList.reduce((acc,x) => acc + x.length, 0)} `)

// 2

const ANSWER_LIST = 'abcdefghijklmnopqrstuvwxyz'.split('')

let currAnswerGroup = ANSWER_LIST

const answerByGroups = []

for (const answer of answerList) {
    if (answer === '') {
        answerByGroups.push(currAnswerGroup.join(''))
        currAnswerGroup = ANSWER_LIST 
    } else {
        const currAnswer = answer.split('')
        const newAnswerGroup = []
        for (const answerGroup of currAnswerGroup) {
            if (currAnswer.findIndex(x => x === answerGroup) !== -1) {
                newAnswerGroup.push(answerGroup)
            }
        }
        currAnswerGroup = newAnswerGroup
    }
}

answerByGroups.push(currAnswerGroup.join(''))

console.log(answerByGroups.join('\n'))
console.log(`Total de réponse : ${answerByGroups.reduce((acc,x) => acc + x.length, 0)} `)
