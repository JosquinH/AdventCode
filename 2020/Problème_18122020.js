...
// Formate l'input pour pouvoir calculer

const formatInput = (input, curr_i = 0) => {
    const opList = []
    let curNum = 0
    let i = curr_i
    ...
    while (i < input.length) {
        ...
        if (regNumber.test(curChar)) {
            curNum = curNum * 10 + parseInt(curChar)
            ++i
        } else if (curChar === '*' || curChar === '+') {
            if (curNum!==0) opList.push(curNum)
            opList.push(curChar)
            curNum = 0
            ++i
        } else if (curChar === '(') {
            const {opList: curList, curr_i} = formatInput(input,i+1)
            opList.push(curList)
            i = curr_i + 1
        } else if (curChar === ')') {
            if (curNum!==0) opList.push(curNum)
            return {opList, curr_i: i}
        } else {
            break
        }
    }
...
const computeOpList = (opList) => {
    const newOpList = []
    for (const op of opList) {
        if (Array.isArray(op)) {
            newOpList.push(computeOpList(op))
        } else {
            newOpList.push(op) 
        }
    }

    let curRes = newOpList[0]
    let curOp
    for (let i = 1; i < newOpList.length; ++ i) {
        let curVal = newOpList[i]
        if (curVal === '+' || curVal === '*') {
            curOp = curVal
        } else {
            if (curOp === '+')  curRes = curRes + curVal
            if (curOp === '*')  curRes = curRes * curVal
        }
    }
...
const computeInput = (input) => {
    const {opList: curList, curr_i} = formatInput(input)
    return computeOpList(curList)
}

let final_res = 0
for (const curInput of input) {
    const res = computeInput(curInput)
    // console.log(`${curInput} = ${res}`)
    final_res += res
}

console.log(`Résultat Final 1 : ${final_res}`)

// Partie 2

const evaluatePlus = (opList) => {
    let curIndex = -1
    const newList = []
    let i = 0
    while (i < opList.length) {
        if (opList[i] === '*') {
            ...
            if (i < opList.length - 1) {
                newList[curIndex] = newList[curIndex] + opList[i + 1]
                i += 2
            } else {
                i +=1
            }
...
const computeOpList2 = (opList) => {
    const newOpList = []
    for (const op of opList) {
        if (Array.isArray(op)) {
            newOpList.push(computeOpList2(op))
        } else {
            newOpList.push(op) 
        }
    }
    const listEvaluatePlus = evaluatePlus(newOpList)
    res = listEvaluatePlus.reduce((acc, x) => acc * x, 1)
    return res
}

const computeInput2 = (input) => {
    const {opList: curList, curr_i} = formatInput(input)
    return computeOpList2(curList)
}

let final_res_2 = 0
for (const curInput of input) {
    const res = computeInput2(curInput)
    console.log(`${curInput} = ${res}`)
    final_res_2 += res
}

console.log(`Résultat Final 2 : ${final_res_2}`)