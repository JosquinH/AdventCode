...
const operationList = input.split('\n').map(x => {
...
console.log(`La valeur de l'accumulateur est de ${accumulator}`)
...
const [nopIndexList, jumpIndexList] = operationList.reduce((acc,x,idx) => {
    if (x[0] === 'nop') {
        acc[0].push(idx)
    } else if (x[0] === 'jmp') {
        acc[1].push(idx)
