...
for (const passeportInput of newInput) {
    if (passeportInput.trim() === '') {
        passeportList.push(currentInput)
        currentInput = []
    } else {
        currentInput.push(passeportInput)  
    }
}
...
for (const passeport of passeportList) {
    const passeportString = passeport.map(x => x.split(' ').join(',')).join(',')
    const objectFields = passeportString.split(',')
    const objPaseport = objectFields.reduce((acc,x) => {
        const res = x.split(':')
        acc[res[0].trim()] = res[1].trim()
        return acc
    }, {} )
    finalInput.push(objPaseport)
}
