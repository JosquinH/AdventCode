...
const numberOfCombinaisonObject = { 0: [input[0], 1] }

for (let i = 1; i < MAX_LENGTH; ++i) {
    const curval = input[i]
    let curRes = curval <= 3 ? 1 : 0

    if (i - 1 >= 0 && curval - input[i - 1] <= 3) curRes += numberOfCombinaisonObject[i - 1][1]
    if (i - 2 >= 0 && curval - input[i - 2] <= 3) curRes += numberOfCombinaisonObject[i - 2][1]
    if (i - 3 >= 0 && curval - input[i - 3] <= 3) curRes += numberOfCombinaisonObject[i - 3][1]

    numberOfCombinaisonObject[i] = [input[i], curRes]
}

console.table(Object.keys(numberOfCombinaisonObject).reduce((acc, x) => { acc.push(numberOfCombinaisonObject[x]); return acc;}, [] ))
console.log(`Nombre total de combinaison : ${numberOfCombinaisonObject[MAX_LENGTH - 1][1]} `)
